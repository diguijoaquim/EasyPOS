    ...
    def initialize_app():
        ...
        try:
            ...
            def show_tables_view():
                ...
                try:
                    # Fetch tables data
                    tables = get_tables()  # Replace with actual API call
                    # ... rest of tables view code ...
                    tables_view = ft.Row(
                        wrap=True,
                        spacing=20,
                        controls=[]
                    ...
                    def filter_tables(e):
                        ...
                        filtered = [
                            ...
                            if (search in str(table["number"]).lower() or
                                search in table.get("guest_name", "").lower() or
                                search in table.get("status", "").lower())
                        ]
                        ...
                        tables_view.controls = [
                            ft.Container(
                                ...
                                content=ft.Column([
                                    ft.Row([
                                        ft.Text(f"T{table['number']}", size=20, weight="bold"),
                                        ft.Text(table.get('time', ''), color=ft.Colors.GREY_700),
                                    ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                                    ft.Container(height=10),
                                    ft.Text(table.get('guest_name', ''), size=16),
                                    ft.Text(
                                        table['status'],
                                        size=14,
                                        color={
                                        ...
                                        }.get(table['status'], ft.Colors.GREY)
                                    ),
                                    ft.Text(f"{table.get('guests', '')} Guests", color=ft.Colors.GREY_700)
                                ]),
                    ...
                    # Inicializa a view com todas as mesas
                    filter_tables(None)  # Chama sem evento para mostrar todas as mesas
                    ...
                    body.content = ft.Column([
                        ft.Container(
                            content=ft.Text(f"Error loading tables: {str(e)}"),
                            alignment=ft.alignment.center
                        )
                    ])
                page.update()
